...
const StudentLogin = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ✅ ส่ง request ไป login พร้อมระบุ Role เป็น "STUDENT"
            const response = await axios.post(`${apiUrl}/auth/login`, {
                username: trimmedUsername,
                password: trimmedPassword,
                role: "STUDENT", // ✨ เพิ่มบรรทัดนี้: บังคับว่าฉันต้องการ Login เข้ามาในฐานะ Student นะ
            });
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center justify-center w-full p-4 student-login-container">
                <div className="fade-in w-full max-w-md">
                    <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
                        {/* Header สีชมพู */}
                        <div className="bg-pink-600 px-8 py-8 text-white relative overflow-hidden">
                            {/* Decorative Circle */}
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-pink-100 text-sm mt-2 ml-11 opacity-90">
                        ...
                        <div className="p-8 pt-10">
                            ...
                            <form className="space-y-6" onSubmit={handleLogin}>
                                {/* Input: Username (รหัสนักศึกษา) */}
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2 ml-1">
                                        ชื่อผู้ใช้งาน / รหัสนักศึกษา
                                    </label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors group-focus-within:text-pink-600 text-gray-400">
                                            <CreditCard size={20} />
                                        </div>
                                        <input
                                            type="text"
                                            ...
                                            className="w-full pl-12 pr-4 py-3.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-100 focus:border-pink-500 outline-none transition-all bg-gray-50/50 focus:bg-white text-gray-800 placeholder-gray-400"
                                            required
                                        />
                                    </div>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full py-4 bg-pink-600 text-white rounded-xl font-bold text-lg hover:bg-pink-700 hover:shadow-lg hover:shadow-pink-200 transition-all transform active:scale-[0.98] mt-2 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {isLoading ? (
                                        <>
                                            <Loader2
                            ...
                            <div className="mt-8 text-center">
                                <Link
                                    href="/role"
                                    className="inline-flex items-center text-sm text-gray-500 hover:text-pink-600 transition-colors gap-2 font-medium"
                                >
                                    <ArrowLeft size={16} />
